...
async def test_composition_selection():
    ...
    result = await client.request("composition:select", {
    ...
    result = await client.request("composition:create", {
        "name": "custom_analyst",
        "type": "profile",
        "role": "data_analyst",
        "model": "sonnet",
...
async def test_agent_self_modification():
    ...
    if spawn_result.get('process_id'):
        ...
        update_result = await client.request("agent:update_composition", {
            "agent_id": "test_researcher_1",
        ...
        await client.request("agent:terminate", {"agent_id": "test_researcher_1"})
...
async def test_dynamic_spawning():
    ...
    # Cleanup
    if spawn_result.get('process_id'):
        await client.request("agent:terminate", {"agent_id": "dynamic_agent_1"})
