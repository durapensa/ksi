#!/usr/bin/env python3
"""
Composition Service Module - Event-Based Version
...
async def get_ksi_context_variable(var_name: str) -> Any:
    """Get KSI context variable via event system."""
    try:
        router = get_router()
        result = await router.emit_first("ksi:context:get", {"variable": var_name})
        if result and not result.get("error"):
            return result.get("value")
        return None
    except Exception as e:
        logger.error(f"Failed to get KSI context variable {var_name}: {e}")
        return None
...
def _resolve_ksi_context_variables(variables: Dict[str, Any], composition: Composition) -> None:
    ...
    for var_name in needed_context_vars:
        ...
        if should_resolve:
            try:
                # Use asyncio to call the async function - this is a blocking call but should be fast
                cached_value = asyncio.run(get_ksi_context_variable(var_name))
