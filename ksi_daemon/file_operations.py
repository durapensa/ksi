...
# Deprecated: FileOperations class for backward compatibility
# TODO: Remove after migrating all usages
class FileOperations:
    """DEPRECATED: Use module-level functions instead"""
    ensure_directories = staticmethod(ensure_directories)
    ...
    create_symlink = staticmethod(create_symlink)
    clean_directory = staticmethod(clean_directory)
    rotate_logs = staticmethod(rotate_logs)


def create_log_entry(entry_type: str, content: Any, **kwargs) -> Dict[str, Any]:
    """
...
def create_human_log_entry(content: str, **kwargs) -> Dict[str, Any]:
    """Create a human input log entry"""
    return create_log_entry("human", content, **kwargs)


def create_claude_log_entry(content: Dict[str, Any], **kwargs) -> Dict[str, Any]:
    """Create a Claude output log entry"""
    return create_log_entry("claude", content, **kwargs)


def create_system_log_entry(message: str, **kwargs) -> Dict[str, Any]:
    """Create a system event log entry"""
    return create_log_entry("system", message, **kwargs)


def create_error_log_entry(error: str, details: Optional[Dict[str, Any]] = None, **kwargs) -> Dict[str, Any]:
    """Create an error log entry"""
    entry = create_log_entry("error", error, **kwargs)
    if details:
        entry["details"] = details
    return entry


# Deprecated: LogEntry class for backward compatibility
# TODO: Remove after migrating all usages
class LogEntry:
    """DEPRECATED: Use module-level functions instead"""
    create = staticmethod(create_log_entry)
    human = staticmethod(create_human_log_entry)
    claude = staticmethod(create_claude_log_entry)
    system = staticmethod(create_system_log_entry)
    error = staticmethod(create_error_log_entry)
