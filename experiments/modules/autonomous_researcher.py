...
try:
    ...
    experiments_base = config.experiments_results_dir
except ImportError:
    # Fallback for standalone usage
    experiments_base = Path("var/experiments/results")
...
class AutonomousResearcher:
    def __init__(self):
        self.experiments_dir = experiments_base
    ...
    def run_experiment_suite(self):
        ...
        status = {
            ...
            "check_files": [
                "var/experiments/results/entropy_report.md",
                "var/experiments/results/concept_graph.json", 
                "var/experiments/results/attractors.json",
                "var/experiments/results/efficiency_analysis.md",
                "var/experiments/results/meta_synthesis.md"
            ]
        }
...
if [ -f var/experiments/results/status.json ]; then
    echo "=== Experiment Status ==="
    cat var/experiments/results/status.json | jq -r '.status'
    echo
fi

echo "=== Latest Observations ==="
ls -la var/experiments/cognitive/*.json | tail -5
echo

echo "=== Experiment Results ==="
for file in var/experiments/results/*.md var/experiments/results/*.json; do
    if [ -f "$file" ]; then
...
if [ -f var/experiments/results/experiment_log.jsonl ]; then
    tail -10 var/experiments/results/experiment_log.jsonl | jq -r '.experiment_name + " -> " + .session_id'
fi
...
def escape_and_research():
    ...
    print("Results will appear in: var/experiments/results/")
    print("="*60)
