...
echo "  tail -f var/logs/daemon/daemon.log.jsonl | grep -E '(tournament_evaluation|match.*complete|Processed tournament)'"
echo ""
echo "Check specific agent responses:"
echo "  ls -lat var/logs/responses/*.jsonl | head -5"
echo ""
echo "Watch for completion:"
echo "  while true; do grep -c \"Match.*complete\" var/logs/daemon/daemon.log.jsonl; sleep 5; done"
echo ""
